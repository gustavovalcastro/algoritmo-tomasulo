...
def fetch_instructions(reorder_buffer, label):
    i = 0
    instructions_base = []
    instructions_base = instructions.copy()
...
def write(reorder_buffer, buff_entry, dependencies, reservation_stations, register_status, branch):
    for id, entry in enumerate(reorder_buffer):
        if buff_entry == entry.entry:
...
def write_instructions(reorder_buffer, reservation_stations, dependencies, register_status, branch):
    for id, station in enumerate(reservation_stations):
        if station.busy == "Yes" and station.changed == "No":
            instr = get_instruction(reorder_buffer, station.dest)

            if instr['remaining_cycles'] <= 0:
                write(reorder_buffer, instr['entry'], dependencies, reservation_stations, register_status, branch)
